    ...
    try {
        if (id) {
            updateData.updateReadableContentFieldsById(id, fields, (error, response) => {
                if (error) {
                    console.error(`Error at route /update_manga_content:`, error);
            ...
            console.error(`Error at route /update_manga_content: No id or content_id provided`);
    ...
    try {
        if (id) {
            updateData.updateWatchableContentFieldsById(id, fields, (error, response) => {
                if (error) {
                    console.error(`Error at route /update_anime_content:`, error);
                    return res.status(500).json({ error: 'Internal server error: ' + error });
                }
            ...
            console.error(`Error at route /update_anime_content: No id or content_id provided`);
            return res.status(400).json({ error: 'Missing id or content_id' });
        }
    } catch (error) {
        console.error(`Error at route /update_anime_content:`, error);
